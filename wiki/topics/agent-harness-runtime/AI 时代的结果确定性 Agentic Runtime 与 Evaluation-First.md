...
后续摄取的 [Agent 系统作为 OS 与 Cloud Runtime 问题](agent-runtime-os-cloud-runtime.md) 可以看作这条线的底层延伸：如果本文回答的是“为什么 runtime + contract 可以提供结果确定性”，那篇材料回答的是“runtime 自身应该怎样像 OS / cloud runtime 一样承载状态、执行隔离、恢复、接口和观测”。
...
- [Agent 系统作为 OS 与 Cloud Runtime 问题](agent-runtime-os-cloud-runtime.md)：那页进一步把 agent runtime 拆成 session log、control plane / data plane、disposable sandbox、stable interface 与 observability。
- [Thin Harness, Fat Skills](thin-harness-fat-skills.md)：那页强调 latent / deterministic 的边界；本文则给出在工程集成中如何把“确定性”落到 eval 与 contract。
- [coding agent 的上下文压缩工作流](coding%20agent%20的上下文压缩工作流.md)：这页更具体讨论复杂代码库里如何通过 compaction、subagent 与 plan 来管理上下文预算。
...
- [AI Architect 的 Advanced Architecture 镜头](../ai-product-product-definition/ai-architect-advanced-architecture.md)：那页把 evaluation set、multi-model hand-off、fallback 与身份边界放进一套更偏课程化的生产升级路径。
