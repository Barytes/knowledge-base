...
- 它和 [本地知识库模式](../context-memory-knowledge-system/local-knowledge-base-patterns.md) 一起说明，长期可用的知识系统不该只是 query-time 检索。
- 它和 [AI 知识系统的产品定义信念](../context-memory-knowledge-system/ai-knowledge-systems-product-definition-beliefs.md) 形成互证：真正高价值的系统，往往先决定默认工作面与蒸馏目标，再决定检索工具怎么接入。
