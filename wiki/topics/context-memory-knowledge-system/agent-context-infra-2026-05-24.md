...
本地已维护的 [volcengine/openviking 仓库地图](volcengine-openviking-repo-map.md) 进一步说明，OpenViking 的重要性不在“又一个向量库”，而在它把 context 从黑盒相似片段升级成可浏览、可定位、可回写的 context runtime。
...
这类 workflow 不依赖单个 vendor，却能立刻改善 brownfield codebase 上的 agent 表现。本地 [coding agent 的上下文压缩工作流](../agent-harness-runtime/coding%20agent%20的上下文压缩工作流.md) 已经把这条线总结得很清楚：上下文窗口不是仓库，而是工作台。
...
这和 [Anthropic 与 OpenAI 的 Agent Systems 履历 North Star](../career-positioning-job-search/Anthropic与OpenAI的Agent%20Systems履历North%20Star.md) 也一致：目标画像不是“做过几个聪明 demo”，而是拥有过可部署、可评估、可迭代、可观测的 agent system。
...
- [Agent 岗位 JD 抽样与能力信号](../career-positioning-job-search/Agent岗位JD抽样与能力信号.md)
