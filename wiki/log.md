...
## [2026-08-26] 维护 | 精简仓库提示词并开放网络证据

将 `AGENTS.md` 收敛为唯一全局政策源，删除重复的 `schemas/AGENTS.md`、Claude 独立规则副本和 Action Coach 平台副本；把 schemas 与 10 个 Skills 改写为目标、硬边界和完成条件，移除固定读取顺序、回答模板、人格诊断与逐步思维脚本。知识查询现在以本地维护知识为默认解释背景，但允许在时效、核验、完整性、原始出处或本地缺口需要时主动搜索网络，并要求区分本地材料、外部证据和 agent 推论。同步修复 repo-map 工具的旧 wiki 目录默认值，并增加提示词单一权威、旧路径、网络搜索政策与启动提示一致性的确定性回归检查。

**更新范围**
- `AGENTS.md`、`CLAUDE.md`、`COMMUNICATION.md`、`notebook/AGENTS.md`
- `schemas/`
- `skills/`
...
## [2026-07-30] correction | 从外部承诺修正为高质量学习场

用户指出，把主要矛盾写成“积累不能转化为外部承诺”仍然停留在浅层；更深的问题可能是理解行业、用“道天地将法”选择位置、刻意练习、建立经结果校准的心智模型，以及是否进入一个好的组织。继续重写 [当前项目收束（2026-07-30）](topics/projects-roadmaps/当前项目收束-2026-07-30.md)：将外部承诺和 runway 降为现实约束与校准信号，把当前工作假设改为“缺少一个把行业事实、真实问题、高手标准、重复训练和结果反馈压在一起的稳定学习场”。新的主线是选择并进入或构造高质量的 Agent Systems 学习场，使“理解行业—选择位置—刻意练习—结果反馈—模型更新”形成复利循环；求职据此成为获得“地、将、法”的可能手段，个人项目则退回单一能力的训练案例。同步在 `contexts/memory/OBSERVATIONS.md` 记录此次过早收束问题。

## [2026-07-30] query | 用主要矛盾重组待办系统

继续更新 [当前项目收束（2026-07-30）](topics/projects-roadmaps/当前项目收束-2026-07-30.md)，把当前主要矛盾明确为“内部产品与技术积累丰富，但在 runway 收缩时还不能稳定转化为外部承诺”。主线因此不是完成一组项目，而是在有限 runway 内获得可重复的岗位、客户或用户承诺，恢复收入与选择权。待办系统改为五层：外部承诺主线、单一当前验证卡、封顶论文债务、产品资产与假设池、按需输入；另用 runway、外部联系、真实任务、复用/付费和论文剩余项观察状态，避免继续用勾选数量冒充方向进展。

## [2026-07-30] correction | 裁员后重做项目与产品探索判断

用户补充已经被 `Lev` 裁员，原有每月 2 万元现金流结束，上一轮把 `Lev` 当作现金流底盘的前提失效。重写 [当前项目收束（2026-07-30）](topics/projects-roadmaps/当前项目收束-2026-07-30.md)，将问题从项目排序改写为“恢复 runway + 封顶论文债务 + 用外部承诺探索产品”。页面进一步识别出 `gogo`、`oh-share-it`、`clawhouse`、日历 + Agent、AI 录音笔和 AI 写作系统是在探索同一个上位命题：Agent 能否跨会话和 deadline 长期持有并推进人的知识工作。后续不为每个名字分别开发 MVP，而是对具体 `ICP × 场景 × 解决方案` 做最低成本排除实验，并以真实任务、复用和付费作为继续条件。同步在 [三年 500 万目标下的职业与生活资源配置](topics/career-positioning-job-search/三年500万目标下的职业与生活资源配置.md) 标注原现金流状态已经过期。

## [2026-07-30] query | 收束当前项目与过期待办

根据 Apple 提醒事项中的 16 个过期或开放条目，并结合当前现金流、论文债务、求职与产品探索处境，新增 [当前项目收束（2026-07-30）](topics/projects-roadmaps/当前项目收束-2026-07-30.md)。结论是：`Lev` 保留为现金流底盘，`NBS` 与 `Stackelberg` 作为有限债务收尾，简历、找工作、行业调研、刷题与内容输入合并成 `Agent Systems Engineer` 市场验证，日历 + Agent 成为唯一主动推进的自有产品试验；`clawhouse`、`oh-share-it`、`gogo`、AI 录音笔与 AI 写作系统不再独立排期。同步更新 [项目 / 路线图 / 执行计划](topics/projects-roadmaps/index.md)。
