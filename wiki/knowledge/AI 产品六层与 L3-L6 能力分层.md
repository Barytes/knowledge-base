...
这和本库已有页面 [AI 时代的结果确定性：Agentic Runtime 与 Evaluation-First](AI 时代的结果确定性 Agentic Runtime 与 Evaluation-First.md) 是直接呼应的。那页强调 runtime 与 contract；这篇则先把“什么才算 runtime 里的 AI”划清边界。
...
这和 [Harness Engineering（约束壳工程）](harness-engineering.md) 那页强调的方向一致。真正难的不是“能不能调到模型”，而是外层控制、验证、权限和协同怎么设计。
...
这也是为什么这页最好和 [AI 时代的结果确定性：Agentic Runtime 与 Evaluation-First](AI 时代的结果确定性 Agentic Runtime 与 Evaluation-First.md) 一起看。前者告诉你“层级加深意味着什么”，后者告诉你“为什么仅有 runtime 还不够”。
...
- 对 [AI 时代的结果确定性：Agentic Runtime 与 Evaluation-First](AI 时代的结果确定性 Agentic Runtime 与 Evaluation-First.md) 来说，它提供了一套更适合给项目或候选人做分层诊断的前置框架。
